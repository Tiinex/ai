...
- If a workflow slice starts depending on a newly added repo tool, update the relevant rolefile `tools` frontmatter in that same slice before treating the workflow as live; do not offload that upkeep to later human reminder.
...
- Keep machine-facing file names split-safe and tokenized by role, model slug or `any`, optional toolset token, optional lifecycle state, and `agent.md`. Prefer forms such as `<role>.gpt-4-1.experimental.agent.md`, `<role>.gpt-4-1.live-feedback-loop.agent.md`, `<role>.gpt-4-1.live-feedback-loop.candidate.agent.md`, or `<role>.any.live-feedback-loop.experimental.agent.md` over dotted model tokens that blur the split boundaries.
