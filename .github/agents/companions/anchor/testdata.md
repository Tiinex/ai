...
- Exact-target-role-label slice: when the target role explicitly says `Anchor (GPT-4.1) (Candidate)`, verify that Anchor resolves that exact label to the candidate file rather than the experimental sibling.
- Split-safe filename slice: when the target role explicitly says `Anchor (GPT-4.1) (Candidate)`, verify that Anchor resolves it through `anchor.gpt-4-1.candidate.agent.md` rather than a literal label search.
- Role-local path slice: after normalization, verify that Anchor attempts `.github/agents/anchor.gpt-4-1.candidate.agent.md` or an equivalent `.github/agents/`-scoped lookup before it gives up on the exact read.
